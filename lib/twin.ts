  ...
  try {
    const intentLine =
      intent.authoringMode === "prompt"
        ...
        : `Brief — ${briefInstruction(intent)}. Mode: ${intent.mode}.`;
...
function mockEngine(text: string, intent: ImportIntent): TwinResult {
  ...
  intent.contentTypes.forEach((typeName, idx) => {
    ...
    const base = {
      ...
      title: `${def.label}: ${briefGoal(intent) || concepts[0] || "Overview"}`,
      concepts: cs.slice(0, 4),
      rationale: `Generated by the mock engine from ${cs.length} concept${cs.length === 1 ? "" : "s"}; ${intent.emphasis} emphasis.`,
      mainLibrary: def.library,
      grounding: qaT[0]?.grounding || sentences[0] || "",
      answerKeyNote:
        intent.mode === "extract"
          ? "Question lifted from the source; answer marked as given in the original."
          : `Key derived from: "${(qaT[0]?.grounding || sentences[0] || "").slice(0, 90)}…"`,
      confidence: (sentences.length > 6 ? "high" : "medium") as "high" | "medium",
      provenance: (intent.mode === "extract" ? "extracted" : "inferred") as
        | "extracted"
        | "inferred",
    };
    const perQ = qaT.map((q) => ({
    ...
    switch (typeName) {
      case "H5P.Summary":
        contentJson = buildSummary(cs);
        break;
      case "H5P.SingleChoiceSet":
        contentJson = buildSingleChoiceSet(qaT);
        questionSignals = perQ;
        break;
      case "H5P.QuestionSet":
        contentJson = buildQuestionSet(qaT);
        questionSignals = perQ;
        break;
